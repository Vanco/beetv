...
data class SearchTypeResult(
    ...
    companion object {
        fun fromSearchTypeResult(result: vanstudio.tv.biliapi.http.entity.search.SearchResultData): SearchTypeResult {
            return when (result.searchTypeResults.first()) {
                is vanstudio.tv.biliapi.http.entity.search.SearchVideoResult -> {
                    SearchTypeResult(
                        videos = result.searchTypeResults.map { Video.fromSearchVideoResult(it as vanstudio.tv.biliapi.http.entity.search.SearchVideoResult) },
                        page = SearchTypePage(nextPageForWeb = result.page + 1)
                    )
                }

                is vanstudio.tv.biliapi.http.entity.search.SearchMediaResult -> {
                    SearchTypeResult(
                        pgcs = result.searchTypeResults.map { Pgc.fromSearchPgcResult(it as vanstudio.tv.biliapi.http.entity.search.SearchMediaResult) },
                        page = SearchTypePage(nextPageForWeb = result.page + 1)
                    )
                }

                is vanstudio.tv.biliapi.http.entity.search.SearchBiliUserResult -> {
                    SearchTypeResult(
                        users = result.searchTypeResults.map { User.fromSearchUserResult(it as vanstudio.tv.biliapi.http.entity.search.SearchBiliUserResult) },
                        page = SearchTypePage(nextPageForWeb = result.page + 1)
                    )
                }
    ...
    data class Video(
        ...
        companion object {
            fun fromSearchVideoResult(video: vanstudio.tv.biliapi.http.entity.search.SearchVideoResult) =
                Video(
                    aid = video.aid,
                    bvid = video.bvid,
                    title = video.title,
    ...
    data class Pgc(
        ...
        companion object {
            fun fromSearchPgcResult(pgc: vanstudio.tv.biliapi.http.entity.search.SearchMediaResult) =
                Pgc(
                    title = pgc.title,
                    cover = pgc.cover,
    ...
    data class User(
        ...
        companion object {
            fun fromSearchUserResult(user: vanstudio.tv.biliapi.http.entity.search.SearchBiliUserResult) =
                User(
                    mid = user.mid,
                    name = user.uname,
